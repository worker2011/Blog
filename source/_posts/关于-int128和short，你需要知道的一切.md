---
title: 关于__int128和short，你需要知道的一切
date: 2023-11-25 14:34:49
tags: C艹
...
2. 不方便。\__int128目前看来是不支持直接读入、输出的。管你是cin、cout、scanf还是printf，都甭想输入一个__int128类型的数据。于是它输入得用string输入字符后$\times 10 +$尾数，输出得数位分解。
