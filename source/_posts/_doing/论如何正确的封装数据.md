...
上面这段儿就是定义了两个 `xxx` 类型的变量 `a` 和 `b`。使用的时候就可以通过 `a.x`、`b.y` 之类的方式来访问啦。
...
咱来解释一下有啥变化。其实就一个：Public（除了这点，Class和Struct没啥区别）。这是因为每一种面向对象语言都有要对其成员进行保护的需求。Class这货默认是Private（你可以理解为要单独写访问函数才能访问，否则 **CE**，不过写工程的时候为保证数据安全，比较常用），要在前面加上`public:`才能正常访问。事实上，C语言中的结构体只能捆绑数据，C++中的和类除了名字不同之外两者其实一样。
...
class OIer{
    ...
    public:
        ...
        string getName() {  // 这个就是访问函数
            return name;  // 注意：访问函数不是官方名称，仅在本博文内有效。
        }
        void haveAge(int a) {
            age = a;
        }
...
我们，既属于"OIer"，又属于"Student"，显然OIer都是学生（现役选手当教练可海星？），所以"OIer"就是"Student"的子类。
```cpp
class Student {
    protected:
        void win() {
...
`pair` 负责把 2 个 变量捆绑到一起，比如小L常用的模板就有这么几行:
```cpp
...
其中这个 `pair<int, int>` 就是了。左右两边就是变量的类型（支持结构体、类），会重载比较符号（即先比较左边的值的大小，一样则比较右边的），这样就可以直接用 `sort` 了。

需要注意的是，`pair` 支持且只支持把两个对象绑定到一起，且只能通过 `first` 和 `second` 访问，有的时候还是自己写结构体方便。
